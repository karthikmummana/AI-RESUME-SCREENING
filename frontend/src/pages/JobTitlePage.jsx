...
export default function JobTitlePage() {
  ...
  const { jobTitle, setJobTitle } = useScreening();
  const [valError, setValError] = useState('');

  const handleContinue = (e) => {
    e.preventDefault();
    if (!jobTitle || !jobTitle.trim()) {
      setValError('Please enter a job title to proceed.');
      return;
    }
    setValError('');
    navigate('/screening/job-description');
  };
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="glass-card p-8 rounded-3xl space-y-6 shadow-2xl border border-slate-800">
        <div className="flex items-center space-x-3 text-brand-400">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Job Position</h1>
            <p className="text-sm text-slate-400">Enter the target position title for resume screening.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleContinue} className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-800/80">
            <button
              type="button"
              onClick={() => navigate('/screening/company')}
